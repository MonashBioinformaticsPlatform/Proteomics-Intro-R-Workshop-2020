...
```{r}
ggplot(nuclear_xl_ms, aes(x = PPIEvidenceInfoGroup)) + geom_bar() + theme_classic()
```

Summarise the interactions by novelty and the evidence for interaction:

```{r}
sum_stats <- nuclear_xl_ms %>% group_by(PPINovelty, PPIEvidenceInfoGroup) %>% summarise(count =  n() ) %>%
  mutate(percent = (count / sum(count)) * 100) %>%
  ungroup() %>%
  mutate(ungrouped_percent =  (count / sum(count)) * 100)
```

```{r}
ggplot(sum_stats, aes(PPINovelty, count)) +
  geom_bar(aes(fill = PPIEvidenceInfoGroup), position = "dodge", stat = "identity") + theme_classic()
```
...
ggplot(example_summary, aes(novelty, count)) +
  geom_bar(aes(fill = evidence), position = "dodge", stat = "identity") + 
  ggtitle("Example dataset") + theme_class()
